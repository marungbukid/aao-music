...
export function determineMusicKey(chordSheet: string) {
  ...
  const pattern =
    '\\b[CDEFGAB](b|bb|#)?(m|maj7|maj|min7|min|sus|add)?(1|2|3|4|5|6|7|8|9)?\\b(#)?';
  ...
  const noteIndex = (index: number) =>
    index < possibleNotes.length ? index : index - possibleNotes.length;
  ...
  notes.forEach((note, index) => {
    scales.forEach((scale) => {
      ...
      scale.int.forEach((noteInt) => {
        score +=
          possibleNotes.indexOf(notes[noteIndex(noteInt + index)]) != -1
            ? 1
            : 0;
      });
